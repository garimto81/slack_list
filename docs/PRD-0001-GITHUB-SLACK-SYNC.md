...
- **수락 기준**:
  - [ ] 진행률(Progress): 프로그래스바 % 업데이트 (예: 50% → 100%)
  - [ ] 비고(Description): PR 제목으로 업데이트
  - [ ] 날짜(Date): 현재 시간으로 업데이트
  - [ ] **Quiet Update**: 채널에 별도 메시지 알림 없음 (기존 리스트 항목만 수정)
  - [ ] 매칭 실패 시 Slack 알림 발송
...
| ID | 업무명 | 진행률 | 비고 |
|:---:|:---|:---:|:---|
| #101 | 소셜 로그인 개발 | 100% | feat(auth): add social login |
...
- [ ] API 키 암호화 저장 (GitHub Secrets)
- [ ] Slack OAuth 토큰 안전 관리
- [ ] PR 데이터 최소 수집 (민감 정보 제외)
...
|:---:|:---|:---:|:---|:---:|:---:|
| #101 | 소셜 로그인 개발 | 100% | feat(auth): add social login | @김개발 | 12/19 |
| #102 | 결제 페이지 개선 | 50% | - | @이개발 | 12/18 |
| #103 | 버그 수정 | 100% | fix: resolve session timeout | @박개발 | 12/19 |
